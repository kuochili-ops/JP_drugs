...
AZURE_LOCATION = "eastasia" # 必須是小寫英文，如 eastasia, southeastasia 或 global
...
# --- 4. Streamlit UI 流程 ---
st.title("💊 醫藥資料全自動處理系統 (強化翻譯版)")

kegg_lookup = get_kegg_master_dict()

uploaded_file = st.file_uploader("1. 上傳導出的 CSV", type="csv")

if uploaded_file:
    df = pd.read_csv(uploaded_file)
    
    if st.button("2. 開始全自動執行 (ID 比對 + 分段翻譯)"):
        # 確保欄位初始化
        df['KEGG_ID'] = df.get('KEGG_ID', "Searching...")
        df['成分名 (英)'] = df.get('成分名 (英)', "N/A")
        df['翻譯理由'] = ""
